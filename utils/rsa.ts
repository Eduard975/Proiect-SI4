...
export async function generateKeyPair(
  bitSizeForGeneratedPrimeNum: number = 8
): Promise<{
...
export function encryptMessage(
  ...
  return message
    .split("")
    .map((char) =>
      modExp(BigInt(char.charCodeAt(0)), publicKey.e, publicKey.n)
    );
}
...
export function decryptMessage(
  ...
  return encryptedMessage
    .map((char) =>
      String.fromCharCode(Number(modExp(char, privateKey.d, privateKey.n)))
    )
    .join("");
}
